...
void spi_dma_test_case(void)
{
    ...
    spi_cfg_dma_0.configuration_b.crc_en = SPI_HARDWARE_CRC_ENABLE;
    /* Set baud rate a bit slower to gurantee the CRC work perfectly */
    spi_cfg_dma_0.configuration_b.baud_rate = SPI_BAUD_RATE_CLK_DIV_128;
    spi_cfg_dma_0.direction = SPI_DIRECTION_1_LINE_TRANSMIT_ONLY;
    spi_cfg_dma_1.configuration_b.crc_en = SPI_HARDWARE_CRC_ENABLE;
    spi_cfg_dma_1.configuration_b.baud_rate = SPI_BAUD_RATE_CLK_DIV_4;
    /* Developer doesn't know why only these 2 baud rate div options can perform CRC smoothly */
    ...
    SPI_Read(&spi_ctrl_dma_1, &dest, 128U);
    SPI_Write(&spi_ctrl_dma_0, &src, 128U);
    ...
    ASSERT(spi_ctrl_dma_0.p_reg->SPI_SR_b.OVR == 0U);
    ASSERT(spi_ctrl_dma_0.p_reg->SPI_SR_b.UDR == 0U);
    ASSERT(spi_ctrl_dma_0.p_reg->SPI_SR_b.CRCERR == 0U);
    ASSERT(spi_ctrl_dma_1.p_reg->SPI_SR_b.BSY == 0U);
    ASSERT(spi_ctrl_dma_1.p_reg->SPI_SR_b.OVR == 0U);
    ASSERT(spi_ctrl_dma_1.p_reg->SPI_SR_b.UDR == 0U);
    ASSERT(spi_ctrl_dma_1.p_reg->SPI_SR_b.CRCERR == 0U);

    spi_cfg_dma_0.configuration_b.crc_en = SPI_HARDWARE_CRC_DISABLE;
    spi_cfg_dma_0.configuration_b.baud_rate = SPI_BAUD_RATE_CLK_DIV_4;
